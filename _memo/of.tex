...
\subsection{Exemple de réduction fonctionnel}
Soit $L$ = $\{(i,j)$ tel que i et h sont des indices de machine déterministes telles que pour tout mot d'entrée $x$, on n'a $M_i(x)=\uparrow$ et $M_j(x)\neq \uparrow \}$
\begin{description}
\item[Montrer que L est RE-difficile] revient à prouver $HALTING \leq_f L$
\item[] $f$ (i,x) $\rightarrow$ (j,k):
\begin{description}
\item[] (i,x) $\in$ HALTING ssi $M_i(x) \neq \uparrow$
\item[] (j,k) $\in$ L ssi $M_j(y) = \uparrow$ et $M_k(y) \neq \uparrow$, $\forall y$.
\end{description}
$\begin{cases}
M_j(y) & boucle \\
M_k(y) & M_i(x)
...
\item[Montrer que L est coRE-difficile] revient à prouver $\neg HALTING \leq_f L$
\item[] $f$ (i,x) $\rightarrow$ (j,k):
\begin{description}
\item[] (i,x) $\in \neg$ HALTING ssi $M_i(x) \neq \uparrow$
\item[] (j,k) $\in$ L ssi $M_j(y) \neq \uparrow$ et $M_k(y) = \uparrow$, $\forall y$.
\end{description}
$\begin{cases}
M_j(y) & y \\
M_k(y) & M_i(x)
